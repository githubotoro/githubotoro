## 👀 Me

👋 I'm Uday, a recent graduate from the University of Southern California with a passion for building products.

☕ Let's have a coffee-chat and create something better together...

- 📧 Reach me via: **uday.khokhariya@gmail.com**
- 🧐 Hunt me on: **[ProductHunt](https://www.producthunt.com/@yupuday)**
- 📡 Connect with me: **[Twitter](https://twitter.com/yupuday)** | **[LinkedIn](https://www.linkedin.com/in/yupuday/)**

## ✍️ Skills

- **Programming**: Python, C/C++, Go, Java, JavaScript, TypeScript, Scala, SQL
- **Cloud & DevOps**: Google Cloud Platform, AWS, Docker, CI/CD, Linux, Git
- **Databases**: PostgreSQL, MongoDB, Supabase, Snowflake, Drizzle, Prisma, GraphQL
- **Frameworks & Libraries**: React, Next.js, Node.js, Nest.js, Express.js, Tailwind

## 🔎 Research

- **DAMBNFT: Document Authentication Model through Blockchain and Non-fungible Tokens**

  - 🔗 **DOI:** [10.1007/978-981-16-9967-2_33](http://dx.doi.org/10.1007/978-981-16-9967-2_33)
  - 📘 **Book:** Smart Trends in Computing and Communications, Proceedings of SmartCom 2022
  - 📚 **Series:** Lecture Notes in Networks and Systems (LNNS, volume 396)

- **Securing Cookies/Sessions Through Non-Fungible Tokens**

  - 🔗 **DOI:** [10.1007/978-3-031-11217-1_10](http://dx.doi.org/10.1007/978-3-031-11217-1_10)
  - 📘 **Book:** Database Systems for Advanced Applications, DASFAA 2022 International Workshops
  - 📚 **Series:** Lecture Notes in Computer Science (LNCS, volume 13248)

## 🚀 Projects (Work + Personal)

- **[Hera](https://ethglobal.com/showcase/hera-54mpb)** - Desktop app for betting on the winner of P2P Fithcade matches.
- **[BAM Protocol](https://ethglobal.com/showcase/bam-protocol-mg10y)** - Primitive for incentivizing provable off-chain actions on-chain.
- **[Royco APIs](https://api.royco.org/api)** - APIs for interacting with the Royco protocol.
- **[Royco Dashboard](https://app.royco.org/)** - UI interface for managing assets on the Royco protocol.
- **[Royco SDK](https://github.com/roycoprotocol/royco-sdk)** - SDK for building apps on top of the Royco protocol.
- **[Payrennial](https://ethglobal.com/showcase/payrennial-p4xtb)** - Venmo for farcaster frames that helps you send and receive money inside Warpcast.
- **[OwnAd](https://ethglobal.com/showcase/ownad-aasop)** - Framework for turning NFTs into Ad-spaces.
- **[Flowy](https://ethglobal.com/showcase/flowy-810ya)** - Methodology for streaming NFTs across chains in real-time.
- **[ETH-Line](https://ethglobal.com/showcase/eth-line-ygmy8)** - Platform for connecting Ethereum addresses via facetime, voice calls and chats.
- **[Formsify](https://formsify.vercel.app/)** - Web3 App for creating decentralized, trustless and distributed forms.
- **[Crypto Chai](https://crypto-chai.vercel.app/)** - Decentralized chai stall illustrating blockchain's immutability factor.
- **[Laxmi Crypt Fund](https://laxmi-crypt-fund.vercel.app/)** - DApp based on lottery analogy.
- **[MegaCryption](https://megacryption.vercel.app/)** - Web App for better understanding, visualization and cryptanalysis of basic cryptography algorithms.
- **[OSVIS](https://osvis.vercel.app/)** - Disk scheduling algorithms visualizer based on the concepts of Operating System.
- **[Ecological Mining on Ethereum](https://colab.research.google.com/github/githubotoro/ecological-mining-on-ethereum/blob/main/Ecological_Mining_on_Ethereum.ipynb)** - Data mining application which implements 'Multi-Linear Regression' model to predict the transaction fee required to mine a new block on Ethereum.
- **[Totorulla](https://opensea.io/collection/totorulla)** - NFT Community of 'Totoru Family' based on Studio Ghibli's production 'My Neighbor Totoro'.

## 🎉 Achievements

- Winner of Lit Protocol Track at ETH San Francisco Hackathon by ETH Global, October 2024
- Winner of Optimism Bounty at FarHack Hackathon by Farcaster, May 2024
- Winner of Top 10 Finalists at Frameworks Hackathon by ETH Global, March 2024
- Winner of Top 3 under DAOs & Communities, one of the Top 15 finalists at ETH Denver Hackathon, February 2024
- Winner of Most Creative Use of Push Protocol at ETH Online Hackathon by ETH Global, October 2023
- Winner of Top 10 by Push Protocol at Scaling Ethereum Hackathon by ETH Global, March 2023
- Winner of Superfluid and Push Protocol tracks at ETH for All Hackathon by ETH India & Devfolio, February 2023
- Winner of Worldcoin & Push Protocol tracks at ETH India Hackathon, December 2022
- Graduate of Buildspace's Nights & Weekends Program, 2022-2023
- Presidential Scholar, Merit Scholarship Recipient, August 2019

## 💼 Work History

- `curr` Full-Stack Engineer @ [Waymont Holdings](https://www.royco.org/)
- `prev` Data Analyst @ [OpenBlock Labs](https://www.openblocklabs.com/)
- `prev` Dev Team Member @ [Blockchain at USC](https://www.blockchainusc.com/)
- `prev` Software Enginer Intern @ [Tanthetaa Software Studio](https://www.tanthetaa.com/)
- `prev` Researcher - Blockchain & NFTs @ [Department of Computer Science & Engineering, SoT, PDEU](https://sot.pdpu.ac.in/ce-dept.html)
- `prev` Builder @ [Buildspace's Nights & Weekends](https://buildspace.so/nights-weekends)
- `prev` Vice-President @ [RANG - The Fine Arts Club of PDEU](https://www.instagram.com/rang.pdeu/)
- `prev` Graphic Design Head @ [GeeksToPeaks - The GFG PDEU Chapter](https://www.instagram.com/geekstopeaks.pdeu/)
